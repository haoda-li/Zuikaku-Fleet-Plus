...
def extract_table(t, interval='bubble_chart'):
    ...
    for i in range(5):
        ...
        while i < len(cell):
            ...
            obj['prerequisite'] = [a.get_text()
                                   for a in cell[i + 1].find_all('a')]
            obj['prerequisite'] = list(
                filter(lambda x: x != "待验证", obj['prerequisite']))
            obj['name_ja'] = cell[i +
                                  2].find('span', {'lang': "ja"}).get_text()
            obj['name'] = cell[i +
                               2].find('span', {'style': "font-weight:bold"}).get_text()
            obj['description_ja'] = cell[i +
                                         3].find('span', {'lang': "ja"}).get_text()
            obj['description'] = cell[i +
                                      3].find('span', {'style': "font-weight:bold"}).get_text()
            try:
...
if __name__ == "__main__":
    res = requests.get(
        "https://zh.kcwiki.org/wiki/%E4%BB%BB%E5%8A%A1#.E5.91.A8.E5.B8.B8.E4.BB.BB.E5.8A.A1")
    ...
    with open('../data/missions.json', 'w', encoding="utf-8") as file:
        json.dump(sorted(list(all_data.values()),
                         key=lambda x: x['id']), fp=file, ensure_ascii=False, indent=2)
